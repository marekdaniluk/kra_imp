...
TEST_CASE("kra_imp_delinearize_with_offset too small output", "[delinearize_with_offset]")
{
    const unsigned int width = 4;
    const unsigned int height = 4;
    const unsigned int pixel_size = 4;
    ...
    output._buffer = output_buffer.data();
    output._buffer_size = output_buffer.size();
    output._width = output_width;
    const kra_imp_error_code_e result = kra_imp_delinearize_with_offset(input_buffer.data(), input_buffer.size(), width, &output);
    REQUIRE(result == KRA_IMP_PARAMS_ERROR);
}

TEST_CASE("kra_imp_delinearize_with_offset too big offset", "[delinearize_with_offset]")
{
    const unsigned int width = 4;
    const unsigned int height = 4;
    const unsigned int pixel_size = 4;
    ...
    output._buffer_size = output_buffer.size();
    output._width = width;
    output._offset = pixel_size;
    const kra_imp_error_code_e result = kra_imp_delinearize_with_offset(input_buffer.data(), input_buffer.size(), width, &output);
    REQUIRE(result == KRA_IMP_PARAMS_ERROR);
}

TEST_CASE("kra_imp_delinearize_with_offset with offset", "[delinearize_with_offset]")
{
    const unsigned int width = 2;
    const unsigned int height = 2;
    const unsigned int pixel_size = 4;
